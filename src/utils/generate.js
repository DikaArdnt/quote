...
import smartcrop from '../Sharp/index.js';
import runes from '../../utils/Runes.js';
import EmojiRegex from '../../utils/EmojiRegex.js';
import { getFile } from '../../utils/Fetcher.js';

const avatarCache = new Map();
const emojiImageJson = JSON.parse(fs.readFileSync('./assets/emoji-apple.json'));
...
class QuoteGenerate {
	...
	async downloadMediaImage(media, mediaSize, type = 'id', crop = true) {
		...
		const load = await getFile(mediaUrl);
		if (crop || mediaUrl.match(/.webp/)) {
			const imageSharp = sharp(load.data);
			...
			for (let index = 0; index < getCustomEmojiStickers.length; index++) {
				...
				loadCustomEmojiStickerPromises.push(
					(async () => {
						...
						if (getFileLink) {
							const load = await getFile(getFileLink).catch(() => {});
							const imageSharp = sharp(load.data);
							const sharpPng = await imageSharp.png({ lossless: true, force: true }).toBuffer();
