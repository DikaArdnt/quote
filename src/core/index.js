...
import QuoteGenerate from './generate.js';
...
export default async parm => {
	...
	if (type === 'quote') {
		...
		const canvasPatternImage = await loadImage(patternImg);
		...
		canvasPicCtx.fillText(parm.watermark || '', canvasPic.width - 25, canvasPic.height - 25);

		if (format === 'webp') quoteImage = await sharp;
		...
		canvasPicCtx.fillText(parm.watermark || '', 0, 0);

		quoteImage = await sharp(canvasPic.toBuffer()).png({ lossless: true, force: true }).toBuffer();
	} else {
		if (format === 'webp') quoteImage = await sharp(canvasQuote.toBuffer()).webp({ lossless: true, force: true }).toBuffer();
		else quoteImage = canvasQuote.toBuffer('image/png');
	}
